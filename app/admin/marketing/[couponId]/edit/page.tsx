...
export default function EditCouponPage({ params }: { params: { couponId: Id<"coupons"> } }) {
    const { couponId } = use<any>(params as any);
    ...
    useEffect(() => {
        if (coupon) {
            ...
            if (coupon.minPurchaseAmount) {
                setMinRequirement("amount");
                setMinPurchaseValue(coupon.minPurchaseAmount.toString());
            } else {
                setMinRequirement("none");
            }

            setCategories(coupon.applicableCategories || []);

            if (coupon.usageLimit) {
                setLimitTotalUse(true);
                setUsageLimitValue(coupon.usageLimit.toString());
            } else {
                setLimitTotalUse(false);
            }

            setLimitPerCustomer(coupon.limitPerCustomer || false);
            setIsActive(coupon.isActive);

            if (coupon.validFrom) {
                setStartDate(new Date(coupon.validFrom).toISOString().split('T')[0]);
            }
            if (coupon.validUntil) {
                setEndDate(new Date(coupon.validUntil).toISOString().split('T')[0]);
            }
        }
    }, [coupon]);
    ...
    const handleUpdateCoupon = async () => {
        ...
        try {
            ...
            console.error(error);
            alert("Failed to update coupon: " + (error as any).message);
        } finally {
    ...
    if (coupon === null) {
        return <div className="p-10">Coupon not found.</div>;
    }

    const isExpired = coupon.validUntil ? Date.now() > coupon.validUntil : false;
        ...
        <main className="flex-1 overflow-y-auto pb-20">
            <div className="max-w-[1024px] mx-auto px-6 lg:px-10 py-8">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                    ...
                    <span className="text-muted-foreground text-sm font-medium">/</span>
                    <span className="text-foreground text-sm font-medium">Edit Coupon</span>
                </div>
                ...
                <div className="flex flex-wrap justify-between items-end gap-3 mb-8">
                    <div className="flex flex-col gap-2">
                        ...
                        <p className="text-muted-foreground text-base font-normal">
                            Update details for coupon <strong>{coupon.code}</strong>.
                        </p>
                    </div>
                </div>
                        ...
                        <div className="bg-card p-6 rounded-xl border border-border shadow-sm">
                            <div className="flex justify-between mb-6">
                                ...
                                <div className="flex items-center gap-2">
                                    <Label htmlFor="is-active-switch" className="text-sm cursor-pointer">Active</Label>
                                    <Checkbox
                                        id="is-active-switch"
                                        checked={isActive}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-muted-foreground text-xs">Changing the code will not affect existing orders.</p>
                                    </div>
                                </div>
                                <div className="space-y-2">
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            {limitTotalUse && (
                                                <p className="text-xs text-muted-foreground">
                                                    Current usage: <strong>{coupon.usageCount}</strong>
                                                </p>
                                            )}
                                        </div>
                                    </div>
                            ...
                            <div className="bg-primary/5 p-6 rounded-xl border-2 border-dashed border-primary/30">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className={isExpired ? "text-destructive font-bold" : (isActive ? "text-green-600 font-bold" : "text-muted-foreground")}>
