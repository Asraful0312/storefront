...
export default function PaymentsSettingsPage() {
    ...
    const handleSaveRule = () => {
        ...
        setTaxRules(prev => {
            ...
            if (index >= 0) {
                // Update existing (though logic typically allows only one rule per region, simplistic here)
                const newRules = [...prev];
    ...
    if (shippingSettings === undefined || taxSettings === undefined) {
            ...
            {isZoneModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-background rounded-lg p-6 w-full max-w-lg space-y-4 shadow-xl border">
                        <h2 className="text-xl font-bold">{editingZone?.id.startsWith("zone-") && !zones.find(z => z.id === editingZone.id) ? "Add Zone" : "Edit Zone"}</h2>
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={e => setEditingZone({ ...editingZone, regions: e.target.value.split(",").map((s: string) => s.trim()) })}
                                ...
                                {editingZone?.rateType === "weight" && (
                                    <div>
                                        ...
                                        <Input
                                            ...
                                            onChange={e => setEditingZone({ ...editingZone, perKgRate: parseInt(e.target.value) || 0 })}
                                        />
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex justify-end gap-2 pt-2">
                            <Button variant="outline" onClick={() => setIsZoneModalOpen(false)}>Cancel</Button>
                ...
                <div className="space-y-6">
                    ...
                    <section className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
                        <div className="p-6 border-b border-border">
                            <h2 className="text-foreground text-xl font-bold leading-tight">Payment Gateways</h2>
                            <p className="text-muted-foreground text-sm mt-1">
                                Connect your store to global payment processors.
                            </p>
                        </div>
                        {/* Stripe Gateway */}
                        <div className="flex items-center gap-4 px-6 min-h-[80px] py-4 justify-between border-b border-border">
                            ...
                            <div className="flex items-center gap-4 shrink-0">
                                <Button variant="outline" size="sm" className="font-bold">Configure</Button>
                        ...
                        <div className="flex items-center gap-4 px-6 min-h-[80px] py-4 justify-between">
                            <div className="flex items-center gap-4">
                                <div className="text-blue-700 flex items-center justify-center rounded-lg bg-blue-700/10 shrink-0 size-12">
                                    <Wallet className="size-6" />
                                </div>
                                <div className="flex flex-col justify-center">
                                    <p className="text-foreground text-base font-bold leading-normal">PayPal</p>
                                    <p className="text-muted-foreground text-sm font-normal leading-normal">
                                        Let customers pay using their PayPal balance or credit cards.
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-4 shrink-0">
                                ...
                                <tbody className="divide-y divide-border">
                                    {zones.length > 0 ? zones.map((zone: any) => (
                                        <tr key={zone.id} className="hover:bg-secondary/30 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-muted-foreground text-sm">{zone.regions.join(", ")}</td>
                                            <td className="px-6 py-4 text-foreground text-sm font-bold">
                                                ${(zone.baseRate / 100).toFixed(2)}
                                                {zone.rateType === "weight" && ` + $${((zone.perKgRate || 0) / 100).toFixed(2)}/kg`}
                                            </td>
                                            <td className="px-6 py-4 text-muted-foreground text-sm">{zone.deliveryTime}</td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button onClick={() => handleEditZone(zone)} variant="ghost" size="icon" className="size-8 text-primary hover:bg-primary/10">
                                                        <Edit className="size-4" />
                                                    </Button>
                                                    <Button onClick={() => handleDeleteZone(zone.id)} variant="ghost" size="icon" className="size-8 text-muted-foreground hover:bg-red-50 hover:text-red-500">
                    ...
                    <section className="bg-card rounded-xl shadow-sm border border-border overflow-hidden mb-12">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            {/* Manual Tax Rules Table (Only if Manual is selected) */}
                            {taxMethod === "manual" && (
                                <div className="border rounded-lg overflow-hidden mt-4">
                                    ...
                                    <table className="w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y">
                                            {taxRules.length > 0 ? taxRules.map((rule) => (
                                                <tr key={rule.region}>
                                                    <td className="px-4 py-3 font-medium">{rule.region}</td>
                                                    <td className="px-4 py-3">{rule.rate}%</td>
                                                    <td className="px-4 py-3 text-right">
                                                        <Button onClick={() => handleDeleteRule(rule.region)} variant="ghost" size="icon" className="size-6 text-muted-foreground hover:text-red-500">
