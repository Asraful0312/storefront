...
import { useCloudinaryImages, ProductImage } from "@/hooks/useCloudinaryImages";
...
export default function EditProductPage() {
    ...
    // Fetch product data
    const product = useQuery(api.products.getWithVariants, { id: productId });
...
function EditProductForm({ product, productId }: { product: any; productId: Id<"products"> }) {
    // Transform API data to form data
    const initialData: Partial<ProductFormData> & { variants?: any[] } = {
        name: product.name,
        ...
        shippingSublabel: product.shippingSublabel,
        warrantyLabel: product.warrantyLabel,
        warrantySublabel: product.warrantySublabel,
        ...
        digitalStockMode: product.digitalStockMode || "unlimited",
        digitalStockCount: product.digitalStockCount,
        giftCardCodeMode: product.giftCardCodeMode || "auto",
    ...
    const initialImages: ProductImage[] = product.images.map((img: any) => ({
    ...
    const cloudinary = useCloudinaryImages({
        productName: form.name,
        initialImages
    });

    const digitalFileUpload = useCloudinaryFiles({
        folder: "digital-files",
        initialFiles: product.digitalFiles || [],
    });
    ...
    const handlePreview = () => {
        ...
        const previewData: ProductDetail = {
            ...
            rating: product.rating || 0, // Mock or real if available
        ...
        try {
            ...
            await form.updateProduct({
                ...
                dimensions: (formData.length && formData.width && formData.height)
                    ...
                    : undefined,
                requiresShipping: form.productType === "physical" ? formData.requiresShipping : false,
                ...
                digitalFiles: digitalFileUpload.files,
                maxDownloads: form.maxDownloads,
                giftCardCodeMode: form.giftCardCodeMode,
            ...
            // 2. Replace Variants
            // Only if variations exist or options are defined.
            // Actually, we should call replace even if empty to clear old variants if user deleted all options.
            if (form.colorOptions.length > 0 || form.sizeOptions.length > 0 || form.variants.length > 0) {
                await form.replaceVariants({
                    productId,
                    variants: form.variants.map((v) => ({
                        size: v.size,
                        colorId: v.colorId, // Using colorId as string ref
                        sku: v.sku,
                        stockCount: v.stockCount,
                ...
                // If no options, should we perform a replace with empty list? 
                // Yes, effectively clearing variants.
                if (initialData.variants && initialData.variants.length > 0) {
                    // If there WERE variants but now cleared, replace with empty.
                    await form.replaceVariants({ productId, variants: [] });
                }
            }
            ...
            console.error(err);
            form.setError(err instanceof Error ? err.message : "Failed to update product");
        } finally {
    ...
    const handleSetMainImage = (publicId: string) => {
        cloudinary.handleSetMainImage(publicId);
    };

    const handleRemoveImage = (publicId: string) => {
        cloudinary.handleRemoveImage(publicId);
    };
                    ...
                    <div className="max-w-6xl mx-auto px-8 py-8">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                {form.productType === "digital" && (
                                    <section className="bg-card rounded-xl p-6 shadow-sm border border-border">
                                        ...
                                        {digitalFileUpload.files.length > 0 && (
                                            <div className="mt-4 space-y-2">
                                                ...
                                                {digitalFileUpload.files.map((file) => (
                                                    <div key={file.publicId} className="flex items-center gap-3 p-3 bg-secondary/50 rounded-lg">
                                        ...
                                        {/* Max downloads */}
                                        <div className="mt-4 space-y-2">
                                            <Label className="text-sm font-medium">Max Downloads per Customer</Label>
                                            <Input
                                                type="number"
                                                placeholder="Unlimited (leave empty)"
                                                value={form.maxDownloads ?? ""}
                                                onChange={(e) => form.setMaxDownloads(e.target.value ? parseInt(e.target.value) : undefined)}
                                                className="h-10"
                                            />
                                ...
                                {/* Gift Card Settings (only for gift cards) */}
                                    ...
                                    <section className="bg-card rounded-xl p-6 shadow-sm border border-border">
                                        ...
                                        <div className="space-y-4">
                                            <div className="space-y-2">
                                                ...
                                                <Select value={form.giftCardCodeMode} onValueChange={(v: "auto" | "manual") => form.setGiftCardCodeMode(v)}>
                                                    <SelectTrigger className="w-full">
                                                        <SelectValue />
                                                    </SelectTrigger>
                                                    <SelectContent>
                                                        <SelectItem value="auto">Auto-generate codes</SelectItem>
                                                        <SelectItem value="manual">Manual entry</SelectItem>
                                                    </SelectContent>
                                                </Select>
                                                <p className="text-xs text-muted-foreground">
                                                    {form.giftCardCodeMode === "auto"
                                                        ? "Unique codes will be generated automatically at purchase"
                                                        : "You will manually enter codes for each order"}
                                                </p>
                                            </div>
                                        </div>
                                    </section>
                                ...
                                {/* Shipping Section (physical products only) */}
                            ...
                            <div className="space-y-6">
                                ...
                                {/* Stock Management (for digital/gift card products) */}
                                {form.productType !== "physical" && (
                                    <section className="bg-card rounded-xl p-6 shadow-sm border border-border">
                                        <div className="flex items-center gap-2 mb-6">
